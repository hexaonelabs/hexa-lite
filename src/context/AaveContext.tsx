...
export const AaveProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const init = async () => {
    ...
    const poolGroups: IPoolGroup[] = poolReserves.reduce((acc, reserve) => {
      const symbol = reserve.symbol;
      const chainId = +(reserve.id.split('-')[0]);
      const borrowApy = reserve.borrowingEnabled ? Number(reserve.variableBorrowAPY) : 0;
