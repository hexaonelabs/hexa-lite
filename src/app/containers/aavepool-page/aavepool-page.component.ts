...
export class AAVEPoolPageComponent implements OnInit {
  ...
  public readonly userReserveData$: Observable<
    | null
  ...
  public readonly userSummary$: Observable<
  ...
  constructor(
    ...
    this.userSummary$ = combineLatest([
      this.token$.asObservable(),
      this._aavev3Service.getUserSummaries$(),
    ]).pipe(
      map(([token, userSummaries]) => {
        if (!token || !userSummaries) {
          return null;
        }
      ...
      filter((userSummary) => !!userSummary),
    );
    this.userReserveData$ = combineLatest([
      this.userSummary$,
      this.marketPool$.asObservable(),
    ]).pipe(
      map(([userSummary, pool]) => {
        if (!userSummary || !pool) {
          return null;
        }
        const userReserveData = userSummary.userReservesData.find(
          (reserve) => reserve.reserve.id === pool.id
        );
        return userReserveData;
      }),
      filter((userReserveData) => !!userReserveData)
    );
  }
  ...
  async choseOption() {
    ...
    const hasDepositPosition = await firstValueFrom(this.userReserveData$).then(
      (value) => Number(value?.underlyingBalance || 0) > 0
    );
    ...
    const userSummary = await firstValueFrom(this.userSummary$);
    if (!userSummary) {
      throw new Error("No user summary found");
    }
    const token = this.token$.value!;
    const aToken: TokenAmount = await getToken(
      this.token$.value?.chainId!,
      this.marketPool$?.value?.aTokenAddress!
    ).then(
    ...
    switch (true) {
      case role === "borrow": {
        const canBorrow = Number(userSummary?.availableBorrowsUSD || 0) > 0;
        if (!canBorrow) {
          const ionAlert = await new AlertController().create({
            header: "Error",
            message: `You have to deposit collateral on ${new ToChainNamePipe().transform(
              this.token$.value?.chainId || 0
            )} network to enable borrow.`,
            buttons: ["OK"],
          });
      ...
      case role === "withdraw": {
        const canWithdraw = await firstValueFrom(this.userReserveData$).then(
          (value) => Number(value?.underlyingBalance || 0) > 0
        );
        if (!canWithdraw) {
          const ionAlert = await new AlertController().create({
  ...
  private async _executeAction(role: string) {
    ...
    await this._aavev3Service.actions(role, {
      pool,
      amount,
    });
  ...
  private async _getMaxAmountToAction(action: string) {
    ...
    switch (true) {
      ...
      case action === "borrow": {
        ...
        maxAmount =
          availableBorrowsUSD / Number(this.marketPool$.value?.priceInUSD);
        break;
      }
      ...
      case action === "repay": {
        const debtToken: TokenAmount = await getToken(
          this.token$.value?.chainId!,
          this.marketPool$?.value?.variableDebtTokenAddress!
        ).then(
          async (token) =>
            (await getTokenBalance(walletAddress!, token)) || token
        );
        const debtTokenAmount =
          Number(debtToken.amount) / 10 ** debtToken.decimals;
        const tokenAmount =
          Number(this.token$.value?.amount) / 10 ** this.token$.value?.decimals;
